                                              ...
                                              std::string& name, std::string& filename)
{
    ...
    if (type != "form-data") return false;
...
bool MultipartReader::initFromContentType(const std::string& ct) {
    ...
    if (!starts_with(ct, "multipart/form-data")) return false;
    std::string::size_type b = ct.find("boundary=");
    if (b == std::string::npos) return false;
    std::string val = ct.substr(b + 9);
    // strip optional quotes
    if (!val.empty() && val[0]=='"') {
        if (val.size() < 2 || val[val.size()-1] != '"') return false;
...
bool MultipartReader::startBoundaryLine(const std::string& line, bool& isFinal) const {
    ...
    if (!starts_with(line, dashBoundary_)) return false;
    if (line.size() == dashBoundary_.size()) { isFinal = false; return true; }
    if (line.size() == endBoundary_.size() && line == endBoundary_) { isFinal = true; return true; }
    return false;
}

std::size_t MultipartReader::feed(const char* data, std::size_t n, IMultipartSink* sink)
{
    if (!data || n==0 || !sink) return 0;
    std::size_t fed = 0;
    buf_.append(data, n);
    fed = n;
    ...
    while (!buf_.empty() && st_ != S_DONE && st_ != S_ERROR) {
        if (st_ == S_PREAMBLE) {
            std::string line;
            if (!findLine(line)) break; // need more
            bool finalB = false;
            if (!startBoundaryLine(line, finalB)) continue; // skip preamble junk lines
            if (finalB) { st_ = S_DONE; break; }
            st_ = S_HEADERS;
            ...
            for (;;) {
                if (!findLine(line)) return fed; // need more
                if (line.empty()) break; // end headers
                std::string::size_type c = line.find(':');
                if (c == std::string::npos) { setError("malformed part header"); return fed; }
            ...
            if (it == hdrs.end()) { setError("no content-disposition"); return fed; }
            if (!parseContentDisposition(it->second, cur_.name, cur_.filename)) {
                setError("bad content-disposition");
                return fed;
            }
            if (!sink->onPartBegin(cur_)) { setError("sink refused part"); return fed; }
            st_ = S_DATA;
        }
        else if (st_ == S_DATA) {
            // Search for "\r\n--boundary"
            ...
            if (m1 != std::string::npos) m = m1;
            if (m2 != std::string::npos && (m==std::string::npos || m2 < m)) m = m2;
            ...
            if (m == std::string::npos) {
                ...
                if (safe) {
                    if (!sink->onPartData(buf_.data(), safe)) { setError("sink write"); return fed; }
                    buf_.erase(0, safe);
                }
            ...
            if (m > 0) {
                if (!sink->onPartData(buf_.data(), m)) { setError("sink write"); return fed; }
                buf_.erase(0, m);
            }
            ...
            // consume --boundary
            if (starts_with(buf_, dashBoundary_)) buf_.erase(0, dashBoundary_.size());

            // final?
            bool isFinal = false;
            if (starts_with(buf_, "--")) { isFinal = true; buf_.erase(0,2); }
            ...
            if (!sink->onPartEnd()) { setError("sink close"); return fed; }
            if (isFinal) { st_ = S_DONE; }
            else { st_ = S_HEADERS; cur_ = IMultipartSink::Part(); }
        }
        else if (st_ == S_AFTER) { st_ = S_DONE; }
        else { break; }
    }
    return fed;
}
