                                    ...
                                    CGIStreamer* cgi_streamer)
{
    ...
    if (!ctx.vs) {
        // No VS resolved → hard error response
    ...
    ctx.matched_prefix = decision.matched_prefix;  // keep full URI mapping intact
    ctx.upstream_name  = decision.upstream_name;

    // Body/temp defaults (set real values elsewhere if you track them)
    ctx.temp_file_used = false;
    ctx.temp_filename  = std::string();

    // Provide connection-owned CGI streamer for async CGI
    ...
    switch (decision.kind)
    {
        ...
        {
            // Build a direct error response so the caller treats this as "done"
        ...
        default:
            // Nothing to do; treat as done (no body)
