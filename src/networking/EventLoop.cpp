...
EventLoop::~EventLoop() {
    for (size_t i = 0; i < _hs.size(); ++i) delete _hs[i];
}
...
bool EventLoop::addFD(int fd, short events, ClientConnection* owner) {
    if (!addFD(fd, events, static_cast<Handler*>(0))) return false;
    owners_[fd] = owner;
    return true;
}
...
void EventLoop::removeFD(int fd) {
    int idx = indexOfFD(fd);
    if (idx >= 0) {
        if (_hs[idx]) { delete _hs[idx]; _hs[idx] = 0; }
        _pfds[idx] = _pfds.back(); _pfds.pop_back();
        _hs[idx]   = _hs.back();   _hs.pop_back();
        owners_.erase(fd);
...
void EventLoop::run(int timeout_ms) {
    ...
    while (!_stop) {
        ...
        for (size_t i = 0; i < dispatch.size(); ++i) {
            ...
            int idx = indexOfFD(fd);
            if (idx < 0) continue; // may have been removed
            Handler* h = _hs[idx];
            if (h) h->onEvent(fd, rev);
        }
    }
}
...
bool EventLoop::setOwner(int fd, ClientConnection* owner) {
    if (indexOfFD(fd) < 0) return false;
    owners_[fd] = owner; return true;
}
