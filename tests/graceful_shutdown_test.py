#!/usr/bin/env python3
"""
Graceful shutdown E2E (subject-compliant)

Validates ONLY the mandatory behaviors:
- Quiesce: listeners closed on SIGTERM (no new connections)
- Reap: hung CGI that never writes headers is reaped (no zombies)
- Exit: server process terminates

We DO NOT require that a streaming CGI keeps draining after SIGTERM.
That behavior is nice-to-have but not mandated by the subject.
"""
...
# Keep slow.py for realism, but its outcome won't gate PASS/FAIL.
...
# hang.py never writes headers; we use this to verify reaping.
...
def hung_request(url, started_evt: threading.Event, done_evt: threading.Event, timeout=120):
    # Just open and block; server should reap it on shutdown.
    try:
...
def main():
    ...
    ap.add_argument("--grace", type=int, default=3, help="informational only")
    args = ap.parse_args()
    ...
    try:
        ...
        # Fire slow stream (best-effort; result is not used for PASS/FAIL)
        ...
        # Start hung CGI request to verify reaping
        hung_started_evt = threading.Event()
        hung_done_evt = threading.Event()
        t_hung = threading.Thread(target=hung_request,
                                  args=(url_hang, hung_started_evt, hung_done_evt, 120),
                                  daemon=True)
        t_hung.start()
        ...
        # Reap check: hang.py should be gone
        ...
        print(f"(Informational) Slow stream: ok={slow_res.ok} status={slow_res.status} bytes={slow_res.bytes} dur={slow_res.duration:.2f}s err={slow_res.err}")

        # Subject-compliant pass criteria: quiesce + reap + exit
        all_ok = quiesce_ok and reap_ok and server_exited
